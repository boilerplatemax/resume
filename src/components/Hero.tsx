...
export default function Hero() {
  return (
    <section
      id="top"
      className="relative px-5 sm:px-6 pt-28 md:pt-36 pb-16 md:pb-20"
    >
      {/* Soft background shapes */}
      <div
        aria-hidden
        className="pointer-events-none absolute -top-40 right-[-10%] h-[36rem] w-[36rem] rounded-full bg-coral-light blur-3xl opacity-70"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute top-60 -left-40 h-80 w-80 rounded-full bg-butter-light blur-3xl"
      />
      ...
      <div className="container-page relative">
        <div className="grid items-center gap-12 md:grid-cols-12 md:gap-10">
          ...
          <div className="order-first md:order-last md:col-span-5">
            <div className="relative mx-auto w-64 sm:w-80 md:w-full md:max-w-[26rem]">
              <div
                aria-hidden
                className="absolute inset-0 translate-x-3 translate-y-3 rounded-[2.5rem] bg-coral"
              />
          ...
          <div className="min-w-0 md:col-span-7">
            ...
            <p className="mt-7 max-w-xl text-lg md:text-xl leading-relaxed text-ink-soft">
              Full-stack developer, designer, and founder. I take products from
              database to brand: I built and monetized{" "}
              <strong className="font-semibold text-ink">UnionTab</strong> to
              2,000+ active users, shipped a sales CRM, and launched 150
            ...
            <p className="mt-8 font-mono text-xs uppercase tracking-[0.16em] text-ink-muted">
              React &middot; Next.js &middot; TypeScript &middot; Postgres
              &middot; Figma
            </p>
          </div>
        </div>
        ...
        <dl className="mt-16 md:mt-24 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-px overflow-hidden rounded-3xl border border-line bg-line">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="bg-white px-5 py-6 last:col-span-2 sm:last:col-span-1"
            >
